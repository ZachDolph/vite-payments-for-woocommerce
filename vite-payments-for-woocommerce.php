...
/*
	...
	Tags: blockchain, vite, woocommerce
...
function vpfw_init_gateway_class()
{
  ...
  class WC_Vite_Gateway extends WC_Payment_Gateway
  {
    ...
    public function __construct()
    {
      ...
      add_shortcode('vitepay_react_app', array($this, 'vitepay_react_app_function'));
    }
     ...
     */
    public function openVitePayWindow()
    {
      $this->processVPFWCheckout(vitepay_react_app_function());
    }

    public function vitepay_react_app_function()
    {
      $vpfwShortcode = '&lt;div style="display: none;" id="vitepay-react-app" >&lt;/div>';
      $vpfwShortcode .= '<script src="' . VPFW_URL . 'includes/vitepay-react-app/build/static/js/main.71e6a95b.js"></script>';
      $vpfwShortcode .= '<script src="' . VPFW_URL . 'includes/vitepay-react-app/build/static/js/main.80fa3147.css"></script>';
      $vpfwShortcode .= '<script>window.amountDefault=' . $this->amountDefault . ' window.tokenDefault=' . $this->tokenDefault . ' window.defaultMemo=' . $this->defaultMemo . ' window.addressDefault=' . $this->addressDefault . ' window.nodeURL=' . $this->nodeURL . ' window.httpURL=' . $this->httpURL . ' window.paymentTimeout=' . $this->paymentTimeout . ' window.onPaymentSuccess=' . $this->onPaymentSuccess . ' window.onPaymentFailure=' . $this->onPaymentFailure . '</script>';
      return $vpfwShortcode;
    }
  }
}
